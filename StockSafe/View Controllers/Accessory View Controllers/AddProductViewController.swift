...
class AddProductViewController: UIViewController {
    ...
    @IBAction func addNewProduct(_sender: UIButton) {
        let pdvc = presentingViewController?.children.first(where: { $0.restorationIdentifier == "productDashboard" }) as? ProductsViewController
        var sl: ShelfLife?
        ...
        if checkProductRestrictions() {
            manager.addNewProduct(name: productName, locations: currentSelections, shelfLife: sl, color: selectedColor) {
                ...
                pdvc!.manager.configureProducts()
                pdvc!.productCollection?.reloadData()
    ...
    private func checkProductRestrictions() -> Bool {
        let nameCell = addProductTable.cellForRow(at: IndexPath(row: 0, section: 0)) as? TextFieldCell
        let blankCheck = nameCell?.textField.checkIfBlank()
        let nameCheck = manager.checkIfNameAlreadyExists(name: nameCell?.textField.text ?? "", field: "name", in: "products")

        let checks = [blankCheck, nameCheck]
    ...
    override func viewDidLayoutSubviews() {
        ...
        addNewProductButton.addShadow()
        addNewProductLabel.addSubtleShadow()
    }
...
extension AddProductViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            ...
            cell.textField.makeRequiredField()
