...
class SelectionView: UIView, ColleagueProtocol {
    ...
    public var selectedLocationKey: [Int:String]?
    
    public var selectedDestination: Location?
    
    public var selectedDestinationKey: [Int:String]?
...
extension SelectionView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        // This code is to ensure that the first cell reflects that it's selected by increasing its borderWidth
        if indexPath.row == 0 {
            cell.isSelected = true
            cell.layer.borderWidth = 2
        }
        // This code is to ensure that the other cells (besides the first cell) do not appear as selected
        else {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .locations:
            ...
            selectedLocationKey = [indexPath.row : locationsToDisplay![indexPath.row].name]
        case .destinations:
            selectedDestination = destinationsToDisplay![indexPath.row]
            selectedDestinationKey = [indexPath.row : destinationsToDisplay![indexPath.row].name]
        }
        if indexPath.row != 0 {
